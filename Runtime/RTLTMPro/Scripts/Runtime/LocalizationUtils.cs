...
public static class LocalizationUtils
{
    ...
    /// <summary>
    /// (Editor only) Gets a locale to use in edit mode in the editor.
    /// </summary>
    /// <param name="tableCollection">
    /// Optional table collection with which to filter the available locales.
    /// </param>
    /// <returns>The locale, null if none usable found.</returns>

    private static Locale Editor_GetValidLocaleInEditMode(LocalizationTableCollection tableCollection)
    {
    ...
    /// <summary>
    /// Gets the localized string from an already loaded table, taking into account whether we are
    /// in edit mode, play mode, or a build.
    /// </summary>
    ...
    {
        // If we are in the editor in edit mode, we need to find a valid locale and get the
        // localized string from it:
#if UNITY_EDITOR
        {
